...
async function updateImagenes() {
    console.log('🖼️  Actualizando imágenes de las piezas...\n');

    try {
        // Actualizar imágenes con URLs relevantes
        ...
        console.log('📦 Actualizando imágenes de piezas...\n');
        
        let actualizadas = 0;
        for (const item of actualizaciones) {
            const { error } = await supabase
                .from('Pieza')
                .update({ ImagenUrl: item.url })
                .eq('Nombre', item.nombre);

            if (error) {
                console.error(`❌ Error en ${item.nombre}:`, error.message);
            } else {
                actualizadas++;
        ...
        console.log(`\n✨ ¡Completado! ${actualizadas} de ${actualizaciones.length} piezas actualizadas`);
